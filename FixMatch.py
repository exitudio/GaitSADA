...
def get_parser():
    ...
    parser.add_argument('--method', type=str,
                        default='CDAN-E', choices=['CDAN', 'CDAN-E', 'CDAN_AMCA', 'CDAN-E_AMCA'])
...
def train_step(src_data, trg_data, s, m, hp_lambda=0):
    ...
    with tf.GradientTape() as tape:
        src_logits = model(src_images,
                            training=True,
                            hp_lambda=hp_lambda)
        trgt_weak_logits = model(trgt_images_weak,
                            training=True,
                            hp_lambda=hp_lambda)
        ...
        pseudo_mask = tf.cast(tf.reduce_max(pseudo_labels, axis=1) >= confidence, tf.float32)
...
def gen_dataset(x_data, y_data):
    ...
    train_datasets.append(ImgGenAnchorDataset(imgen_weak, imgen_strong, x_data, y_data, batch_size=batch_size, u=1))

    # if arg.trgt_aug>0:
    #     train_datasets.append(imgen.flow(X_train_src, y_train_src, batch_size=batch_size))
    #     limit = x_data.shape[0]//batch_size
    #     train_datasets.append(range(limit))
    # else:
...
if __name__ == '__main__':
    ...
    shutil.copy2(inspect.getfile(ImgGenAnchorDataset), arg.log_dir)
    ...
    # optimizer = tfa.optimizers.SGDW(weight_decay=0.0005, momentum=0.9, nesterov=True, learning_rate=learning_rate)
